...
/***********************************************************************************************************************************
**  Module GroceryList Implementation
**
***********************************************************************************************************************************/
module GroceryList;                                                                         // Primary Module Interface Definition
import std;
import GroceryItem;
...
GroceryList::GroceryList_Ex::GroceryList_Ex( const std::string_view message, std::source_location location )
  : std::logic_error{ CSUF::CPSC131::exceptionString( message, std::move(location), 3 ) }   // skip the derived class's constructor, this constructor, and the exceptionString function
{}
...
void GroceryList::insert( const GroceryItem & groceryItem, Position position )
{
  ...
  else                                     throw std::logic_error{ CSUF::CPSC131::exceptionString("Unexpected insertion position",    // Programmer error.  Should never hit this!
...
void GroceryList::insert( const GroceryItem & groceryItem, std::size_t offsetFromTop )                // insert provided grocery item at offsetFromTop, which places it before the current grocery item at offsetFromTop
{
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      /// Hint:  "Vector.cppm" in our Sequence Container Implementation Examples shows you how to do this.  (wink wink)
    ...
    throw CapacityExceeded_Ex("Array capacity exceeded");

    std::shift_right(_gList_array.begin(), _gList_array.begin() + offsetFromTop, _gList_array_size - offsetFromTop);
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ///////////////////////// TO-DO (5) //////////////////////////////
      ...
      /// has a function called std::next() that does that, or you can use simple pointer arithmetic to calculate it.  "Vector.cppm"
...
_gList_dll.insert(it, groceryItem);
    /////////////////////// END-TO-DO (6) ////////////////////////////
  } // Part 3 - Insert into doubly linked list
...
auto it = std::next(_gList_sll.before_begin(), offsetFromTop);
...
std::shift_left(_gList_array.begin() + offsetFromTop + 1, _gList_array.end(), _gList_array_size - offsetFromTop - 1);
_gList_array_size--;
    /////////////////////// END-TO-DO (8) ////////////////////////////
  } // Part 1 - Remove from array
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    /// return the results of comparing the sizes using three-way comparison.
    ///
    ///
    /// The content of all the grocery lists's containers is the same - so pick an easy one to walk.
std::weak_ordering GroceryList::operator<=>(const GroceryList& rhs) const {
  auto common_size = std::min(size(), rhs.size());
  for (std::size_t i = 0; i < common_size; ++i) {
    if (auto cmp = _gList_vector[i] <=> rhs._gList_vector[i]; cmp != 0) {
      return cmp;
    }
  }
...
std::ostream & operator<<( std::ostream & stream, const GroceryList & groceryList )
{
  ...
  return stream << std::format( "{}", groceryList );
}
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  ///////////////////////// TO-DO (18) //////////////////////////////
    /// Extract grocery items from the provided stream and insert them at the bottom of the provided grocery list until end-of-file.
